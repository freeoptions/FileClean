...
Assert-Contains -Path $mainWindowCode -Pattern "CenterFocusedResultInView" -Message "Result list must center the current preview row when selection changes."
Assert-Contains -Path $mainWindowCode -Pattern "ScrollIntoView\(item\)[\s\S]*ContainerFromItem\(item\)" -Message "Centered selection must realize the virtualized row before calculating its position."
Assert-Contains -Path $mainWindowCode -Pattern "TransformToAncestor\(ResultsScroll\)" -Message "Centered selection must calculate the selected row position relative to the results viewport."
Assert-Contains -Path $mainWindowCode -Pattern "Math\.Clamp\(centeredOffset, 0, ResultsScroll\.ScrollableHeight\)" -Message "Centered selection must clamp offsets at the start and end of the list."
Assert-Contains -Path $mainWindowCode -Pattern "requestVersion != _centerSelectionRequestVersion" -Message "Stale centered-selection requests must not override newer selections."
Assert-Contains -Path $mainWindowCode -Pattern "ScrollIntoView\(item\)[\s\S]*UpdateLayout\(\)[\s\S]*ScrollToVerticalOffset\(clampedOffset\)" -Message "Virtualized realization and centered scrolling must complete in one render pass to prevent visible jumping."
Assert-NotContains -Path $mainWindowCode -Pattern "DispatcherPriority\.ContextIdle" -Message "Centered selection must not expose an edge-aligned frame before the final offset is applied."
...
Assert-Contains -Path $mainWindowXaml -Pattern "#E3F0FF" -Message "Current preview row must use a distinct cool-blue highlight."
Assert-Contains -Path $mainWindowXaml -Pattern "TargetType=`"DataGridCell`"[\s\S]*IsPreviewFocus[\s\S]*#E3F0FF" -Message "Current preview blue highlight must be applied at cell level."
Assert-Contains -Path $mainWindowXaml -Pattern "IsPreviewFocus[\s\S]*#4B8FD8[\s\S]*3,0,0,0" -Message "Current preview row must include a clear blue leading accent."
